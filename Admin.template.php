...
function template_admin()
{
	...
	foreach ($context[$context['admin_menu_name']]['sections'] as $area_id => $area)
	{
		echo '
						<div class="title_bar">
							<h4 class="titlebg">', $area['title'], '</h4>
						</div>
						<div id="group_', $area_id, '" class="windowbg admin_group">';
		...
		foreach ($area['areas'] as $item_id => $item)
		{
			...
			if (!empty($item['icon_file']))
				...
				echo '
							<a href="', $url, '"><span class="main_icons ', $item['icon_class'], !empty($item['inactive']) ? ' inactive' : '', '"></span>', $item['label'], '</a>';
		}
