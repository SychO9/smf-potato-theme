...
function template_pm_popup()
{
	...
	echo '
		...
		<div class="pm_unread dropdown-resources', empty($context['unread_pms']) ? ' dropdown-resources--empty' : '', '">';
	...
	else
	{
		foreach ($context['unread_pms'] as $id_pm => $pm_details)
			echo '
			<a href="', $scripturl, '?action=pm;f=inbox;pmsg=', $pm_details['id_pm'], '" class="unread_notify">
